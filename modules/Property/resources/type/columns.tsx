...
export const TypeColumns = (currentLocale :string): ColumnDef<Type>[] => {
    ...
    return [
        ...
        },
        {
            ...
            cell: ({row}) => {
                return (
                    <Avatar>
                        <AvatarImage className="h-12 w-12" src={row.getValue('image')} alt={(row.getValue('type') as Record<string, string>)[currentLocale]} />
                    </Avatar>
                )
            },
        ...
        },
        {
            ...
            cell: ({row}) => {
                const type : TranslatableField = row.getValue('type')
                return (
                    <div className="flex space-x-2">
                        <span className="max-w-[500px] truncate font-light">{type[currentLocale] || t('not_translated')}</span>
                    </div>
                );
            },
